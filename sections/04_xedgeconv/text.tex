...
\section{Implementation and Ablations}
    ...
    \vspace{-6pt}\paragraph{SymPermutation} As our first concept, we implement rotation symmetric and permutation invariant kernels in two variants: (1) A symmetric kernel which contains only 4 trainable values as shown in \figureref{fig:concept} (denoted as \emph{SymPermutation (full)}). (2) A symmetric kernel, which only has the center and its six neighbours as trainable parameters resulting in 2 trainable values (denoted as \emph{SymPermutation (6-nbh)}). The latter variant is closer related to our proposed method, as \emph{XEdgeConv} also only includes six neighbours in our experiments.
...
\section{Experiments and Results}
    ...
    \begin{figure}[t]
        ...
        {
            \includegraphics[width=\linewidth]{geomed2022_figure_xedge_bubble.pdf}
        }
        \vspace{-24pt}
    \end{figure}
    ...
    \tableref{tab:results} (left) highlights the differences across methods and shows that XEdgeConv can drastically reduce the required model capacity and complexity to each state-of-the-art performance. 
    ...
    The detailed numerical results in \tableref{tab:results} demonstrates the very accurate results that can be obtained with our model that has 15$\times$ fewer parameters than the baseline across all anatomical structures (with a small exception of the stomach, for which rotational invariance seems to be a disadvantage). \figureref{fig:visual} clearly shows the benefit of our model when applied to permuted input data, where the performance of the baseline nnUNet drops to nearly half (45.1\% vs. 84.4\%) but our XEdgeConv method retains its high scores (81.6\% vs. 78.8\%).  
    ...
    \begin{table}
        ...
        {
            \vspace{-18pt}\resizebox{\textwidth}{!}{\begin{tabular}{lS[table-format=2.1]r|ccccccccc|cc|ccc}
    ...
    \end{table}
    \begin{figure}[ht]
        \floatconts{fig:visual}
        {
             \vspace{-20pt}\caption{Prediction on a normally orientated scan (left) and a permuted input (right). 
             Our XEdgeConv method maintains performance. 
             }
        }
        {
            \includegraphics[width=.85\linewidth]{geomed2022_figure_xedge_visual.pdf}
        }
        \vspace{-24pt}
    \end{figure}
    ...
    \begin{table}
        \floatconts{tab:results_mmwhs}
        {
            \caption{Dice overlap of 6 unseen cardiac scans under RIA orientation domain shift (training with AIL orientation). Labels: left myocardium \textcolor{left_myocardium}{$\blacksquare$}, left atrium \textcolor{left_atrium}{$\blacksquare$}, left ventricle \textcolor{left_ventricle}{$\blacksquare$}, right atrium \textcolor{right_atrium}{$\blacksquare$}, right ventricle \textcolor{right_ventricle}{$\blacksquare$}, ascending aorta \textcolor{ascending_aorta}{$\blacksquare$} and pulmonary artery \textcolor{pulmonary_artery}{$\blacksquare$}. 
            }
        }
        {
            \vspace{-18pt}\resizebox{\textwidth}{!}{%
    ...
    \end{table}
    For this setting, we experience a large improvement in Dice mean accuracies, shown in \tableref{tab:results_mmwhs}. The visual results of one test case are shown in \figureref{fig:visual_mmwhs} for the centre slice and the 3D volume (ground truth, nnUNet full-kernel baseline and XEdgeConv). 
    The potential of our method can also be seen in cardiac segmentation where the heavy-baseline nnUNet cannot overcome the orientation domain gap as successful as our method (28.1\% vs. 59.3\%, \tableref{tab:results_mmwhs}). 

    Test case predictions are more convincing for some classes (e.g. the left atrium and pulmonary artery in \figureref{fig:visual_mmwhs}).
    
    \begin{figure}[h]
        \floatconts{fig:visual_mmwhs}
        {
            \vspace{-20pt}\caption{Heavy-baseline and XEdgeConv predictions given a RIA oriented test case. Networks were trained on AIL oriented data. With our method more reasonable predictions are achieved (see left atrium and right ventricle prediction, arrow). Class labels see \tableref{tab:results_mmwhs}.}
        }
        {
            \includegraphics[width=\linewidth]{mmwhs_overview.pdf}
        }
    \end{figure}
