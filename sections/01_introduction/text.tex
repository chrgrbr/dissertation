\section{Motivation} % p 2.5
    ...
    \section{Objectives} % p 1.5
        ...
        \paragraph{Levels of Exploration}
            This thesis will evaluate the achieved improvements two-fold: On the one hand, the clinical level outcome is presented and the gained improvements that generalization enables are highlighted. This is also reflected by the organization of the thesis presented in the next section \ref{sec:organization}.
            On the other hand the technical level that enables or improves generalization for volumetric medical deep learning is discussed. In this thesis, technical findings are presented again on different levels:
            On a global level, acquisition process modeling as well as training strategies are examined. On a smaller scale, individual parts of the training and inference pipeline such as training loss function changes for curriculum learning are presented. Even individual parameter/kernel-level changes of the model are optimized to enable generalization by model design. Looking at the data fed to the model, augmentation schemes on the image intensity-level are developed.
        ...
        \section{Organization and Contributions}  % p 3.5
            ...
            \begin{itemize}
                ...
                \item
                    Chapter \ref{chap:deepstaple} explains concepts of data aggregation and curriculum learning for generalizing on the data level. Applied to the task of vestibular schwanomma tumour and cochlea segmentation for differently weighted MR images, the method was developed to weight individual samples during the training phase of the model. This enables to learn a generalized shape representation given noisy, probably imperfect input labels derived by image registration. The method was published in:
            ...
            In the last chapter of this thesis \ref{chap:summary} \nameref{chap:summary} all of the findings are summarized again two-fold: The clinical outcome is evaluated and further possibilities of the methodological improvements are evaluated with respect to the clinical benefit.

            Technical improvements are further elaborated on, possibly not only yielding insight for medical volumetric deep learning but also other fields of deep learning research.
